            ...
            List<Person> people = new List<Person>()
{
    ...
    { Dogs = new List<Dog>() {dogs[9], dogs[14], dogs[15] } }
            ...
            List<string> firstNamesStartingWithR = people.OrderByDescending(person => person.Age)
                                                          .Select(person => person.FirstName)
                                                          .Where(person => person.StartsWith('R')).ToList();
            firstNamesStartingWithR.ForEach(x => Console.WriteLine(x));

            //Find and print all brown dogs names and ages older than 3 years, ordered by Age - ASCENDING ORDER
            List<Dog> brownDogsOlderThan3 = dogs.OrderBy(dog => dog.Age).Where(dog => dog.Age > 3 && dog.Color == "Brown").ToList();
            brownDogsOlderThan3.ForEach(x => Console.WriteLine($"Name: {x.Name}, Age: {x.Age}"));
            //Find and print all persons with more than 2 dogs, ordered by Name - DESCENDING ORDER
            List<string> peopleWithMoreThan2Dogs = people.OrderByDescending(person => person.FirstName).Where(person => person.Dogs.Count > 2).Select(person => person.FirstName).ToList();
            peopleWithMoreThan2Dogs.ForEach(name => Console.WriteLine(name));

            //Find and print all Freddy`s dogs names older than 1 year
            List<string> freddysDogsOlderThan1 = people.SingleOrDefault(person => person.FirstName == "Freddy").Dogs.Where(dog => dog.Age > 1).Select(dog => dog.Name).ToList();
            freddysDogsOlderThan1.ForEach(dogName => Console.WriteLine(dogName));
            //Find and print Nathen`s first dog
            List<Dog> nathensFirstDog = people.Where(person => person.FirstName == "Nathen").ToList().Select(dog => dog.Dogs.FirstOrDefault()).ToList();
            nathensFirstDog.ForEach(dogsName => Console.WriteLine(dogsName.Name));

            //Find and print all white dogs names from Cristofer, Freddy, Erin and Amelia, ordered by Name - ASCENDING ORDER
            var allWhiteDogsNames = people.Where(person => person.FirstName == "Cristofer" || person.FirstName == "Freddy" || person.FirstName == "Erin" || person.FirstName == "Amelia")
                                    .SelectMany(person => person.Dogs).ToList().Where(dog => dog.Color == "White").OrderBy(dog => dog.Name).ToList();
            allWhiteDogsNames.ForEach(dogName => Console.WriteLine(dogName.Name));
            Console.ReadLine();
        }
    }
}
